...
const aadharRegExp = /(^[0-9]{4}[0-9]{4}[0-9]{4}$)|(^[0-9]{4}\s[0-9]{4}\s[0-9]{4}$)|(^[0-9]{4}-[0-9]{4}-[0-9]{4}$)/
...
export const resetPassSchema = Yup.object({
    newPassword: Yup.string().min(6, 'Password must be 6 characters long').matches(/[0-9]/, 'Password requires a number').matches(/[a-z]/, 'Password requires a lowercase letter').matches(/[A-Z]/, 'Password requires an uppercase letter').matches(/[^\w]/, 'Password requires a symbol').required("This is a required field"),
    confirmPassword: Yup.string().required("This is a required field").oneOf([Yup.ref('newPassword'), null], 'Must match "password" field value')
})